...
public class OrderHistory extends Activity {
	...
	OrderHistoryListAdapter cla;

	// create arraylist variables to store data from server
	static ArrayList<Long> Order_ID = new ArrayList<Long>();
	static ArrayList<String> Category_name = new ArrayList<String>();
	// static ArrayList<String> Category_image = new ArrayList<String>();

	String OrderHistoryAPI;
	int IOConnect = 0;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		cla = new OrderHistoryListAdapter(OrderHistory.this);

		// category API url
		OrderHistoryAPI = Utils.OrderHistoryAPI + "?accesskey="
				+ Utils.AccessKey + "&user_id="
				+ LogedUser.getInstance().getID();
		...
		listCategory.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				// go to menu page
				Intent iMenuList = new Intent(OrderHistory.this,
						MenuListHistory.class);
				iMenuList.putExtra("order_id", Order_ID.get(position));
	...
	public class getDataTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			if ((Order_ID.size() > 0) && (IOConnect == 0)) {
	...
	public void parseJSONData() {
		...
		try {
			...
			HttpUriRequest request = new HttpGet(OrderHistoryAPI);
			...
			for (int i = 0; i < data.length(); i++) {
				...
				JSONObject category = object.getJSONObject("Order");

				Order_ID.add(Long.parseLong(category.getString("order_id")));
				Category_name.add(category.getString("order_date") + " - "
						+ category.getString("total_amount") + " USD "
						+ category.getString("payment_method"));
